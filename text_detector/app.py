...
if __name__ == '__main__':
    ...
    yolo_model_path = 'weights/last.pt'
    if not os.path.exists(yolo_model_path):
        print("=" * 60)
        print("⚠️  ВНИМАНИЕ: Модель YOLO не найдена!")
        print("=" * 60)
        print(f"Ожидаемый путь: {os.path.abspath(yolo_model_path)}")
        print("\nМодель YOLO (last.pt) НЕ устанавливается через requirements.txt.")
        print("Вам нужно:")
        print("  1. Получить обученную модель YOLO (файл last.pt)")
        print("  2. Поместить её в папку: weights/last.pt")
    ...
    if yolo_model is None:
        ...
        try:
            ...
            print("  - Модель YOLO не найдена: weights/last.pt")
            print("  - Проблемы с доступом к HuggingFace")
            print("\nПопробуйте:")
            print("  1. Проверить интернет-соединение")
            print("  2. Убедиться, что модель YOLO находится в weights/last.pt")
